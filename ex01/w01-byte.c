...
int main() {
    ...
    size_t n = 0;
    while (n < size) {
        ssize_t r = write(fd, buf, 1); /* Operates directly with the operating system */
        /* Does not buffer in memory; each call sends the data directly, bypassing the memory cache. */
        if (r != 1) {
            perror("write");
            exit(1);
        }
    ...
    close(fd); /* Does not handle buffers; writes directly to the file. */
    report(n, tstamp() - start);
    fprintf(stderr, "\n");
}
